...
void tcp_in(buf_t *buf, uint8_t *src_ip) {
    ...
    uint32_t data_sz = buf->len - tcp_hdr_sz;
    ...
    switch (tcp_conn->state) {
        ...
        case TCP_STATE_ESTABLISHED:
            ...
            tcp_conn->ack = remote_seq + data_sz;
    ...
    if (data_sz != 0) {
        ...
        if (func == NULL) {
            buf_t icmp_buf;
            buf_init(&icmp_buf, buf->len);
            memcpy(icmp_buf.data, buf->data, buf->len);
            icmp_buf.len = buf->len;

            buf_add_header(&icmp_buf, sizeof(ip_hdr_t));
            icmp_unreachable(&icmp_buf, src_ip, ICMP_CODE_PORT_UNREACH);
        } else {
            buf_remove_header(buf, sizeof(tcp_hdr_t));
            (*func)(tcp_conn, buf->data, buf->len, remote_ip, remote_port);
        }
    }
